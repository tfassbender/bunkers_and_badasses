...
import net.jfabricationgames.bunkers_and_badasses.error.CommunicationException;
...
public class CommunicationSecurity {
	...
	public void secureMessage(Serializable message) {
		if (message instanceof SecurableMessage) {
			...
			throw new CommunicationException("The message sent can't be secured because it doesn't implement SecurableMessage.");
		}
	}
	/**
	...
	private void resendMessage(int messageId) {
		if (client != null) {
			try {
				...
				throw new CommunicationException("Couldn't resent a message.", cce);
			}
		}
		else {
			try {
